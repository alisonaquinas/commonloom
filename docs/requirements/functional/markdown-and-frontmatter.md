...
| CLR-FUNC-009 | Commonloom shall preserve current Markdown behavior when no flavor is configured. | The default flavor policy behaves like the current CommonMark plus GFM parser path. |
| CLR-FUNC-010 | Commonloom shall apply the selected flavor consistently across parsing, rendering, source traces, links, and media extraction. | Parser, renderer, and trace outputs come from the same effective flavor state. |
