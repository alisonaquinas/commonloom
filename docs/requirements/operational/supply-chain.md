...
| CLR-OPS-069 | Dependabot version updates shall be enabled for npm dependencies and GitHub Actions. | `.github/dependabot.yml` schedules grouped version updates for npm workspaces and workflow actions against `develop`, with cooldowns for newly released versions and separate major-update grouping. |
| CLR-OPS-070 | Dependabot security updates shall be configured separately from routine version updates. | `.github/dependabot.yml` keeps npm security updates on the default branch path with daily checks and labels security PRs separately from routine version-update PRs. |
