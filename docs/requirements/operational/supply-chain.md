...
| CLR-OPS-069 | Dependabot version updates shall be based on `develop` for npm dependencies and GitHub Actions. | `.github/dependabot.yml` schedules grouped version updates for npm workspaces and workflow actions with `target-branch: develop`, npm SemVer cooldowns, a GitHub Actions default cooldown, and separate major-update grouping. |
