...
| CLR-TECH-021 | Commonloom shall use the unified ecosystem for Markdown and HTML processing. | Implementation uses `unified`, `remark-parse`, `remark-gfm`, `remark-rehype`, `rehype-raw`, `rehype-sanitize`, and `rehype-stringify` or documented successors, with `gray-matter` owning frontmatter extraction. |
